...
from video_demo.identity import Tracklet, associate_tracklets, prioritize_reviews
...
class IdentityTests(unittest.TestCase):
    ...
    def test_review_queue_limits_candidates_per_new_track(self):
        tracks = [Tracklet(f"T{i}", i * 1000, i * 1000 + 500,
                           (1.0, 0.01 * i)) for i in range(5)]
        result = associate_tracklets(tracks, merge_threshold=1.0, review_threshold=0.75)
        self.assertEqual(len([item for item in result.review_candidates if item.track_id == "T4"]), 3)

    def test_uncalibrated_identical_vectors_do_not_auto_merge(self):
        tracks = [Tracklet("T1", 0, 1000, (1.0, 0.0)),
                  Tracklet("T2", 2000, 3000, (1.0, 0.0))]
        result = associate_tracklets(tracks, merge_threshold=1.0,
                                     review_threshold=0.75, merge_enabled=False)
        self.assertEqual(result.track_to_person, {"T1": "P1", "T2": "P2"})
        self.assertEqual(result.review_candidates[0].possible_person_id, "P1")

    def test_cross_scene_candidate_survives_local_fragment_limit(self):
        tracks = [Tracklet(f"T{i}", i * 1000, i * 1000 + 500,
                           (1.0, 0.01 * i)) for i in range(5)]
        result = associate_tracklets(tracks, merge_threshold=1.0, review_threshold=0.75,
                                     max_review_candidates=5)
        selected = prioritize_reviews(result, {"T0": 0, "T1": 0, "T2": 0,
                                                "T3": 0, "T4": 1}, per_category=1)
        self.assertEqual(len([x for x in selected if x.track_id == "T4"]), 1)
