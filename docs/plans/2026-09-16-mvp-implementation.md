...
**当前状态：** Python 3.11.16 和虚拟环境已就绪；PyTorch 2.9.1+cu128 在 L20 上运行小张量求和成功，系统 Python 未修改。服务器细节见 `docs/server-environment.md`。
...
**当前状态：** 已实现固定时间片、证据帧抽取、Qwen3-VL 适配层、CLI、JSON/Markdown/manifest 输出及部分处理标记。Windows 与服务器均运行 18 项测试通过。Qwen3-VL-8B-Instruct 已在 L20 上完成 207.234 秒样例的真实推理，产出可解析 JSON、中文摘要、11 个连续片段和 44 张证据帧。结构闭环已通过，动作细节与结果性描述仍需改进；见 `docs/sample-run-2026-09-17.md`。
...
**当前状态：** 已实现独立于模型的人物 tracklet 关联规则：禁止时间重叠轨迹合并、低于高阈值不自动合并、候选接近时请求人工复核，阈值由调用方提供。尚未接入人体检测、轨迹生成和外观向量模型；不能据此声称样例已有人物 ID 结果。
...
- 已有用户提交的样例视频 `data/example/xzg_314700.mp4` 可用于技术冒烟测试；抽看 10、60、120、180 秒画面，固定机位下有多人反复入镜，适合后续检验视频内匿名人物 ID。此抽看不构成人物关联效果验收。
