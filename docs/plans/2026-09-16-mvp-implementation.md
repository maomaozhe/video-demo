...
**当前状态：** 已实现固定时间片、证据帧抽取、Qwen3-VL 适配层、CLI、JSON/Markdown/manifest 输出及部分处理标记。Windows 用替代模型运行 18 项测试通过（含模型失败日志及人物关联规则）；服务器上一版代码通过 12 项，CLI 帮助页可正常启动。真实 Qwen3-VL 推理待 PyTorch、依赖和权重安装后验证。此阶段的事件仍是片段描述，未达到人物关联和细粒度动作验收。
